...
internal class InvocationRoutingSystemTest {

    @Test
    fun `should route all invocations requests in trace`(){
        ...
        val seed = 212400812412312

        var requestedInvocations: Long = 0
        for (entry in traceSet){
            for (fragment in entry.workload.image.compHistory)
                requestedInvocations += fragment.invocations
        }
        ...
        forEach { experimentConfig ->
            ...
            experiments.add(
                    Scenario(experimentConfig.nameWithoutExtension,
                            "Invocation-routing-system-test",
                            traceSet,
                            config.vmSetup,
                            config.pricingModel,
                            config.failureModel,
        ...
        for (report in reports) {
            assertEquals(requestedInvocations, report.get().invocations, "Not all submitted invocation requests have been handled")
        }
    }
}
